...
# Load the trained ElasticNet model if it exists
try:
    with open('elasticnet_regression_model.pkl', 'rb') as f:
        model = pickle.load(f)
...
def train():
    ...
    model = ElasticNet(alpha=1.0, l1_ratio=0.5)  # You can adjust alpha and l1_ratio parameters
    model.fit(X_train, y_train)
    
    # Save the model
    with open('elasticnet_regression_model.pkl', 'wb') as f:
        pickle.dump(model, f)
